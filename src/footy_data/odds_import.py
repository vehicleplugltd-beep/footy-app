...
def normalise_total_2_5_prices(
    external: pd.DataFrame,
    footy_metrics: pd.DataFrame,
    bookmaker: str,
    source: str,
    ...
    home_team_col: str = "HomeTeam",
    away_team_col: str = "AwayTeam",
    over_odds_col: str = "B365>2.5",
    under_odds_col: str = "B365<2.5",
) -> tuple[pd.DataFrame, OddsImportReport]:
    """Reconcile and normalize historical Over/Under 2.5 decimal prices."""
    if price_kind not in {"open", "close", "snapshot"}:
        raise ValueError("price_kind must be open, close, or snapshot.")

    required_external = {
        date_col, home_team_col, away_team_col, over_odds_col, under_odds_col,
    }
    missing = required_external - set(external.columns)
    if missing:
        raise ValueError(f"Odds CSV missing columns: {sorted(missing)}")
    ...
    ext["home_key"] = ext[home_team_col].map(_team_key)
    ext["away_key"] = ext[away_team_col].map(_team_key)
    ext["price_over"] = pd.to_numeric(ext[over_odds_col], errors="coerce")
    ext["price_under"] = pd.to_numeric(ext[under_odds_col], errors="coerce")
    ext = ext.dropna(
        subset=["match_date_key", "price_over", "price_under"]
    )
    ext = ext[
        (ext["price_over"] > 1)
        & (ext["price_under"] > 1)
