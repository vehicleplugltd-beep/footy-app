...
def command_quality_evaluate(args: argparse.Namespace) -> None:
    ...
    if args.season:
        wanted = {str(value) for value in args.season}
        history = history[
            history["season"].astype(str).isin(wanted)
        ].copy()
    if history.empty:
    ...
    snapshot_prices = snapshot_prices[
        snapshot_prices["match_id"].astype(str).isin(valid_match_ids)
    ].copy()
    close_prices = close_prices[
        close_prices["match_id"].astype(str).isin(valid_match_ids)
    ].copy()
    if snapshot_prices.empty or close_prices.empty:
        raise RuntimeError("Snapshot and closing price histories are required.")
    ...
    if args.market == "1X2":
        ...
        summary, _ = summarize_qualified_1x2(
            assessed,
            outcomes,
        )
    elif args.market == "TOTAL_2.5":
        ...
        summary, _ = summarize_qualified_total_2_5(
            assessed,
            outcomes,
        )
    else:
    ...
    required_metric_columns = ["xg", "xga", "goals", "goals_conceded"]
    data_completeness = float(
        history[required_metric_columns].notna().all(axis=1).mean()
    )
    ...
    if not previous.empty:
        previous = previous[
            previous["model_version"].astype(str) != str(args.model_version)
        ]
        if not previous.empty:
    ...
    snapshot = QualitySnapshot(
        ...
        mean_clv=mean_clv,
        realized_roi=(
            None if summary.get("roi") is None else float(summary["roi"])
        ),
...
def main() -> None:
    ...
    quality_evaluate = sub.add_parser(
        "quality-evaluate",
        help="Evaluate stored model probabilities and price selection, then persist the quality gate",
    )
    quality_evaluate.add_argument("--league", required=True)
    quality_evaluate.add_argument("--season", action="append")
