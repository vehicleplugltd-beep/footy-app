...
export async function getBettingWorkspaceData() {
  ...
  const [dbScopeMatches, outputs, validations, liveOdds, feedRows, recentMatches, discoveredMatches, qualityRows] =
    await Promise.all([
      rest<MatchRow>(
        `footy_matches?select=match_id,kickoff_at,league,home_team,away_team&league=in.(${CORE_MODEL_LEAGUES.map(encodeURIComponent).join(",")})&kickoff_at=gte.${encodeURIComponent(scopeStart)}&kickoff_at=lte.${encodeURIComponent(horizon)}&order=kickoff_at.asc&limit=1000`,
      ),
      rest<ModelRow>(
  ...
  const internationalHistory = Array.from(
    new Map([...internationalHistoricalCandidates, ...statsBombHistory]
      .filter((match) => isInternationalCompetition(match.league) &&
        !/club friendly|champions league|europa league|conference league|club world cup|youth league|u-?1[579]|u-?2[013]|women/i.test(match.league))
  ...
  const internationalPriceIds = new Set(internationalFixtures.filter((match) =>
    liveOdds.some((quote) => sameFixture(match, { match_id: quote.match_id ?? "", kickoff_at: quote.commence_time, league: "", home_team: quote.home_team, away_team: quote.away_team })),
  ).map((match) => match.match_id));
