-- Append-only model quality history. The runtime writes with service_role only.
create table if not exists public.footy_model_quality_snapshots (
  ...
  data_completeness double precision not null check (
  ...
  gate_status text not null check (
    gate_status in ('READY','LIMITED','BLOCKED')
  ),
  validation_status text not null check (
    validation_status in ('APPROVED','WATCH','PASS')
  ),
...
create index if not exists idx_footy_model_quality_league_market
  on public.footy_model_quality_snapshots(league, market, evaluated_at desc);

create index if not exists idx_footy_model_quality_status
  on public.footy_model_quality_snapshots(gate_status, evaluated_at desc);
